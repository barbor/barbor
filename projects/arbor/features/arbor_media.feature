...
Funcionalidade: Preenchimento de campos de texto
  ...
  Esquema do Cenario: Acesso à página do formulário
    ...
    #Quando o botão <Ação de Mídia> é pressionado
    #Quando a opção <Opção de Mídia> é ativada
    Quando o botão de mídia é pressionado
    E a aba de Biblioteca é selecionada
    E a primeira mídia é selecionada
    E é definido para o campo <Campo> o valor <Valor>
    E o botão <Ação> é pressionado
    Exemplos:
      | Página do Formulário | Opção de Mídica    | Campo   | Valor                        | Ação     |
      | 'node/add/article'   | 'Selecionar mídia' | 'Title' | 'Teste automático de artigo' | 'Salvar' |
